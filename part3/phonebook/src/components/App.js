...
function doesNotExist(check, newobj) {
  for (var i = 0; i < newobj.length; i++) {
    if (newobj[i].name === check) {
      console.log('yeah its duplicate')
      return (
        false
      )
    }
...
const App = () => {
  ...
  const addDeet = (event) => {
    ...
    if (newobj.length === 0 || doesNotExist(newName, newobj)) {
      ...
      apiserv
        .posty(newPerson)
        .then(response => {
          console.log('added a dude with', totalid)
  ...
  return (
    <SubForm setMessage={setMessage} message={message} setErr={setErr} err={err} addDeet={addDeet} setPersons={setPersons} newName={newName} newNum={newNum} handleName={handleName} handleNum={handleNum} persons={persons} EachDeet={EachDeet} />
  )
